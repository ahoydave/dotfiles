...
❌ **TOO VERBOSE**:
```markdown
## Verify Unity Asset Meta Files
**Context**: Unity project - during research or verification phase, when documenting asset management
**Rule**: ALWAYS verify .meta files exist for all Unity assets and document any missing files
**How**:
1. List all asset files in Assets/ directory
2. For each asset file, check corresponding .meta file exists
3. Document any missing .meta files in current_system.md
4. Note that .meta files are auto-generated by Unity and shouldn't be manually created

**Why**: Unity uses .meta files to track asset GUIDs. Missing meta files cause "missing reference" errors.
**Added**: 2025-11-23
```
...
- spec/diagrams/[name].puml: Created diagram, generated SVG
