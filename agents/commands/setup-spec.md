...
Initialize a `spec/` folder in the current project with documentation standards.

## Steps

1. Create `spec/README.md` with the content below (verbatim - do not modify it)
2. Create `spec/current-system.md` with the stub below
3. Check whether a `.claude/CLAUDE.md` exists in the project. If it does, and it doesn't already reference spec files, add a pre-task reading section directing agents to read `spec/README.md` and `spec/current-system.md` before any task. If no project CLAUDE.md exists, offer to create one.

Report what was created when done.

---

## spec/README.md content
...
> Replace this stub with an accurate overview of the system. Follow the standards in `spec/README.md`.
...
[C4 diagram or prose description of the major components and how they interact.]

## Key Components

[One section per major component. Each section: short summary of responsibility, then link to detail doc if warranted.]

## Data Flow

[How information moves through the system for the primary use cases.]
